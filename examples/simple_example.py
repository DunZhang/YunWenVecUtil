...
if sys.platform.startswith("linux"):
    model_dir = "/nlp/NLPData/PublicPretrainedModel/SimBERT_ZhuiYi_Torch"
else:
    model_dir = r"D:\Codes\PretrainedModel\simbert_torch"
sen_encoder = BERTSentenceEncoder(model_dir,
                                  device,
                                  pooling_modes=["cls"],
                                  batch_size=8)
### 为target生成句向量库
...
# vec_db.vector vector属性就是句向量了，只需要句向量的可以存储起来了
### 开始搜索
...
# 刚才已经生成vec_db了，也可以通过函数find_topk_by_vecs去搜索
### 输出结果
for i in res:
    print(i)
